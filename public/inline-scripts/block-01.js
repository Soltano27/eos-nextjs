(function() {
  ...
  var NAV_GROUPS = {
    'about':'about','about-legacy':'about','about-mission':'about','about-pillars':'about','about-team':'about',
    'programs':'programs','programs-culture':'programs','programs-guild':'programs','programs-cbhi':'programs',
    'neuroed':'neuroed','neuroed-elearning':'neuroed','neuroed-fellowship':'neuroed','neuroed-alumni':'neuroed',
    'innovation':'innovation','innovation-mima':'innovation',
    'resources':'resources','resources-brain-age':'resources','resources-healthy':'resources','resources-firstaid':'resources','resources-receipts':'resources',
    'media':'media','media-impact':'media','media-research':'media','media-gallery':'media','media-news':'media',
    'cortex':'cortex','cortex-article-1':'cortex',
    'contact':'contact'
  };

  // ── CORE NAVIGATE ──
  // keepNavOpen: true = called from mobNavParent (parent tap) — keep menu open to show children
  window.eosNavigate = function(route, keepNavOpen) {
    if (!route) return;
    ...
    document.querySelectorAll('.nav-item > a[data-route]').forEach(function(a) {
      var r = a.getAttribute('data-route');
      if (r === group || r === route || (NAV_GROUPS[r] && NAV_GROUPS[r] === group)) {
        a.classList.add('active');
      }
    });

    // ── MOBILE NAV ACTIVE ──
    document.querySelectorAll('.mob-nav-child, .mob-nav-standalone').forEach(function(el) {
      el.classList.remove('mob-active');
    });
    document.querySelectorAll('[data-route="' + route + '"]').forEach(function(el) {
      if (el.classList.contains('mob-nav-child') || el.classList.contains('mob-nav-standalone')) {
  ...
  document.addEventListener('click', function(e) {
    ...
    if (el.getAttribute('onclick') && !el.classList.contains('mob-nav-child') && !el.classList.contains('mob-nav-standalone') && !el.classList.contains('mob-nav-cta')) {
      var oc = el.getAttribute('onclick');
      if (oc.indexOf('ps2') !== -1 || oc.indexOf('platSwitch') !== -1 || oc.indexOf('switchPortal') !== -1) return;
    }
    e.preventDefault();
